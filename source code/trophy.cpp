...
HRESULT CTrophy::Init(void)
{
	...
	for (int count_trophy_size = 0; count_trophy_size < (int)CHappenig::HAPPENING_TYPE::MAX - 1; count_trophy_size++)
	{
		...
		if (trophy_flag[count_trophy_size])
		{
			...
			{
			case 0:
				pIcon = CObject2D::Create(D3DXVECTOR3((SCREEN_WIDTH / SCREEN_DIVISION_X_COUNT) + (TROPHY_ICON_X_FEELING * count_trophy_size), SCREEN_HEIGHT / SCREEN_DIVISION_Y_COUNT, 0.0f),
