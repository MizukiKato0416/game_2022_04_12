...
HRESULT CModel::Init(D3DXVECTOR3 pos, D3DXVECTOR3 rot, CXload::X_TYPE type)
{
	...
	//Xファイル情報読み込み
	m_pBuffMat = CManager::GetInstance()->GetXload()->GetBuffMat(type);
	m_nNumMat = CManager::GetInstance()->GetXload()->GetNumMat(type);
	m_pMesh = CManager::GetInstance()->GetXload()->GetMesh(type);
	...
	for (int nCntMat = 0; nCntMat < (int)m_nNumMat; nCntMat++)
	{
		...
		{
			for (int nCntTex = 0; nCntTex < CTexture::GetNum(); nCntTex++)
			{
				std::string sPas = CTexture::GetPas(nCntTex);
				if (strcmp(sPas.c_str(), pMat[nCntMat].pTextureFilename) == 0)
				{
					m_pTexture[nCntMat] = CTexture::GetTexture(nCntTex);
				}
			}
		}
	}
